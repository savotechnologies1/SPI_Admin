...
//   const [period, setPeriod] = useState<Period>("daily");
//   const [chartData, setChartData] = useState<ChartItem[]>([]);
//   const [parts, setParts] = useState<PartItem[]>([]); // Parts state
//   const [totalInventoryCost, setTotalInventoryCost] = useState<number>(0);
//   const [loading, setLoading] = useState<boolean>(false);
...
//         setTotalInventoryCost(
//           rawChartData[rawChartData.length - 1].totalInventoryCost,
//         );
//       } else {
//         setTotalInventoryCost(0);
//       }
...
//                   {/* Time to deliver ki jagah logic ke hisab se extra stock dikha rahe hain */}
//                   {part.leadTime > 1
//                     ? `${part.leadTime} days`
//                     : `${part.leadTime} day`}
//                 </td>
...
//           <span>
//             1-{parts.length} of {parts.length}
//           </span>
...
interface PartItem {
  ...
  costPerUnit: string;
  totalExtraCost: string;
  leadTime: number; // Agar leadTime use kar rahe hain
}
...
const Inventory = () => {
  ...
  const fetchInventoryGraph = async () => {
    try {
      ...
      // Summary se ratio aur cost nikalna
      ...
      setTurnoverRatio(summary.turnoverRatio || 0); // Ratio set kiya
      setOutOfStockCount(summary.outOfStockCount);
    } catch (error) {
      console.error("Inventory graph error:", error);
      setChartData([]);
      setParts([]);
