...
const AddProcess: React.FC = () => {
  ...
  const {
    ...
    setError,
    clearErrors,
  } = useForm<IProcessForm>();

  const navigate = useNavigate();

  // 3. Type the onSubmit handler
  const onSubmit: SubmitHandler<IProcessForm> = async (data) => {
    try {
      // Transform data to match ProcessData type
      ...
      const response = await addProcess(transformedData);
      // Assuming response has a status property
      if (response && (response as any).status === 201) {
        navigate("/process-list");
      }
    } catch (error) {
