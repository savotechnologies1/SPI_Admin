...
const EditWorkInstruction = () => {
  ...
  // --- यहाँ मुख्य बदलाव है (Process Name के साथ Machine Name जोड़ना) ---
  const processOptions = useMemo(
    () =>
      processData.map((item) => ({
        value: item.id,
        label: `${item.name} (${item.machineName || "N/A"})`, // Label logic like Add page
      })),
    [processData],
  );
      ...
      <FormikProvider value={formik}>
        <Form onSubmit={handleSubmit}>
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <div className="w-full sm:w-1/3">
              <label className="font-semibold">Select Process (Machine)</label>
              <Select
                options={processOptions}
                value={
                  processOptions.find(
              ...
              <>
                {values.steps.map((step, index) => {
                  ...
                  return (
                    <div
                      key={step.id || index}
                      className="bg-white p-6 mb-6 rounded-xl relative border shadow-sm"
                    >
                      <div className="absolute top-4 right-4">
                        <FaTrash
                          className="text-red-500 cursor-pointer h-5 w-5"
                          ...
                          {videoSrc && (
                            <div className="mt-2">
                              <video
                                ...
                                width="100%"
                                controls
                                className="rounded-md border h-40"
                              >
                                <source src={videoSrc} />
                              </video>
                            </div>
                ...
                <div className="flex justify-end gap-4">
                  <button
                    ...
                    onClick={() =>
                      arrayHelpers.push({
                        title: "",
                        stepNumber: (values.steps.length + 1).toString(),
                        workInstruction: "",
                        workInstructionImg: [],
                        workInstructionVideo: null,
                      })
