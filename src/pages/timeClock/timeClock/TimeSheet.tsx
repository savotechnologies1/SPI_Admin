...
//   );
// };
// Import icons if you use them, or use plain text
...
const TimeSheet: FC = () => {
  ...
  const handleNextPage = () => pagination?.hasNext && setCurrentPage((prev) => prev + 1);
  const handlePreviousPage = () => pagination?.hasPrevious && setCurrentPage((prev) => prev - 1);
...
const handleApprove = async (date, employeeId) => {
  try {
    const res = await axiosInstance.post(`${BASE_URL}/api/admin/approve-timesheet`, {
  ...
  return (
    <div className="p-4 sm:p-6 mt-6 bg-gray-50 min-h-screen">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Time List</h1>
        <p className="text-sm text-gray-500 mt-1">Dashboard • timeO'clock • time Sheet</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
                {tableHeaders.map((header) => (
                  <th key={header} className="px-4 py-3 text-left text-[11px] font-bold text-gray-600 uppercase">
                    {header}
                  </th>
                ))}
              </tr>
                ...
                timeSheetData.map((entry, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm font-bold text-gray-800">{entry.employeeName}</div>
                      <div className="text-xs text-gray-500">{entry.employeeEmail}</div>
                    </td>
                    <td className="px-4 py-4 text-xs text-gray-600">{entry.loginTime || "-"}</td>
                    <td className="px-4 py-4 text-xs text-gray-600">{entry.logout || "-"}</td>
                    ...
                    <td className="px-4 py-4">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                        entry.vacationStatus === "APPROVED" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                      }`}>
                    ...
                    <td className="px-4 py-4">
                      <button
                        onClick={() => handleApprove(entry.date, entry.employeeEmail)}
                        className="flex items-center gap-1 px-3 py-1 bg-green-600 text-white text-xs font-bold rounded hover:bg-green-700 transition-all shadow-sm"
                      >
                        Approve
                      </button>
                    </td>
