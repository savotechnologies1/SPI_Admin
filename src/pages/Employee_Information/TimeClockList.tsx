...
// 1. Updated Interfaces to match the data used in JSX
interface CustomerItem {
  id: string;
  name: string;
  email: string;
    ...
    <div className="p-4 mt-5 md:p-7">
      <div>
        ...
        <div className="flex flex-wrap items-center mt-2 gap-1 md:gap-2">
          ...
          <span className="text-sm md:text-base">Employees</span>
          <FaCircle className="text-[4px] md:text-[6px] text-gray-500" />
          <span className="text-sm md:text-base text-gray-400">Time Clock</span>
        </div>

        <div className="rounded-md mt-4 bg-white shadow-sm border border-gray-100">
          <div className="p-2 md:p-4">
            <div className="flex flex-col sm:flex-row items-center gap-2 md:gap-4 p-2 md:p-4">
              <div className="flex flex-col w-full sm:w-auto border rounded-md p-1">
                <select
                  className="block w-full sm:w-40 md:w-52 rounded-md border-none text-xs md:text-sm outline-none bg-transparent"
                  value={selectedProcess}
                  onChange={handleProcessChange}
                >
                  <option value="">Select Process</option>
              ...
              <div className="flex-1 w-full relative border p-2 md:p-3 rounded-md">
                <input
                  type="text"
                  placeholder="Search by Name, Email ..."
                  className="w-full rounded-md pl-7 text-xs md:text-sm outline-none"
                  value={searchVal}
                  onChange={handleSearchChange}
                />
                <img
                  src={search_2}
                  alt=""
                  className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 opacity-40"
                />
              </div>
            </div>
          </div>
                ...
                {customerData.length === 0 && !loading ? (
                  ...
                  customerData.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-4 text-xs md:text-sm hidden sm:table-cell">
                        {item.process}{" "}
                        <span className="text-gray-400">
                          ({item.machineName})
                        </span>
                      </td>
                      <td className="px-3 py-4 text-xs md:text-sm font-medium">
                        {item.hours}
                      </td>
                      <td className="px-3 py-4 text-xs md:text-sm hidden md:table-cell">
                        {item.createDate
                          ? format(new Date(item.createDate), "MM/dd/yyyy")
                          : "N/A"}
                      </td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <div className="flex justify-center gap-4">
              <button
                onClick={() => setIsOpen(false)}
                className="flex-1 py-2 bg-brand text-white rounded-md font-medium"
              >
                Confirm
              </button>
              <button
                onClick={() => setIsOpen(false)}
                className="flex-1 py-2 text-red-600 rounded-md border border-red-100 font-medium"
              >
                Cancel
              </button>
            </div>
