...
const CashFlowLineGraph: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // API se dailyCashFlow object ko array mein convert karna
      const rawData = data.dailyCashFlow || {};
      const schedule = Object.entries(rawData).map(([date, cashNeeded]) => ({
        date,
        cashNeeded: Number(cashNeeded) || 0,
      }));

      setAllSchedule(schedule);

      // Default month set karna (Current Month)
      const today = new Date();
  ...
  // Mahine ke hisab se filter karna aur missing dates ko 0 se bharna
  useEffect(() => {
    if (!selectedMonth || allSchedule.length === -1) return;
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const existing = allSchedule.find((s) => s.date === dateStr);
